# Movies Tracker - AI Agent Instructions

This file provides universal instructions for AI coding agents working on the Movies Tracker project. It follows the AGENTS.md standard maintained by the Agentic AI Foundation.
...
**Live Demo**: https://movies-trackers.vercel.app/
...
```
1. User submits login form → login() in src/lib/auth-actions.ts
...
5. Subsequent requests use ensureUser() from src/lib/actions.ts
6. User ID extracted from JWT payload for database operations
```

#### Key Authentication Files

- `src/lib/auth-actions.ts`: Login/logout/register functions
- `src/lib/auth-utils.ts`: Password hashing and JWT utilities
- `src/lib/actions.ts`: `ensureUser()` middleware for Server Actions
...
- **Approach**: Tailwind classes only, no inline styles
...
3. **Authentication**: JWT-based (custom implementation)
...
- `src/components/movie-card.tsx`: Movie display component
- `src/components/theme-toggle.tsx`: Theme switching

### Pages

- `src/app/[locale]/page.tsx`: Home page
- `src/app/[locale]/movie/[id]/page.tsx`: Movie details
- `src/app/[locale]/watchlist/page.tsx`: User watchlist
...
1. **Understand the architecture**: Review the big picture diagram above
...
**Note**: This file follows the AGENTS.md standard. For agent-specific configurations, see:
- `.agents/` - Universal Agents directory structure
- `.trae/` - TRAE-specific rules
- `.opencode/` - OpenCode-specific rules
